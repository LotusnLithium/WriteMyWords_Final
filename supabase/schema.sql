-- ==============================================================================
-- WriteMyWords Complete Database Schema & Security Setup for Supabase
-- ==============================================================================
-- Run this script in your Supabase project's SQL Editor (Dashboard -> SQL Editor -> New query)
-- It creates the profiles, requests, messages tables, triggers, views, RLS policies,
-- and enables Supabase Realtime.
...
-- ==============================================================================
-- PUBLIC BOARD VIEW (Sanitized view without contact info or private details)
-- ==============================================================================
...
-- ==============================================================================
-- AUTOMATIC PROFILE CREATION TRIGGER (When a new user signs up in auth.users)
-- ==============================================================================
...
-- (Enforces open -> claimed -> delivered -> approved states)
-- ==============================================================================
...
BEGIN
  ...
  -- Prevent modifying request ID or original poster
  IF NEW.id <> OLD.id OR NEW.user_id <> OLD.user_id THEN
    RAISE EXCEPTION 'Cannot modify request identifier or owner';
  END IF;

  -- Service role / Edge Functions bypass user checks (used by Razorpay payment verification)
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    ELSIF OLD.status = 'claimed' AND NEW.status = 'delivered' THEN
      ...
      END IF;
      IF NEW.delivery_text IS NULL OR trim(NEW.delivery_text) = '' THEN
        RAISE EXCEPTION 'Delivery text cannot be empty';
      END IF;
    ...
    -- Transition: open/claimed -> cancelled
